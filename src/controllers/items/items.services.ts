import {
  ...
  deleteItem,
  updateItem,
  updateItemWithMultipleParams,
  selectItemsFromList,
  selectItemFromList,
  selectItemById,
} from "../../models/postgres/sqlQueries";
...
class ItemServices {
  ...
  static async getAllItems(listId: string): Promise<ItemServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: rowCount === 0 ? `No items from list ${listId}` : `All items from list ${listId}`,
  ...
  static async getAnItem(itemId: string, listId: string): Promise<ItemServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: rowCount === 0 ? "This item does not exist" : `Item retrieved from list ${listId}`,
      payload: { rows, rowCount },
    };
  }
  ...
  static async getAnItemById(itemId: string): Promise<ItemServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: rowCount === 0 ? "This item does not exist" : "item returned",
      payload: { rows, rowCount },
    };
  }
