...
class ItemServicesMongo {
  ...
  static async getAllItems(listId: string): Promise<ItemServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: items?.length === 0 ? `No items from list ${listId}` : `All items from list ${listId}`,
      payload: { rows: items || Object.assign([]), rowCount: items?.length },
    };
  }
  ...
  static async getAnItem(itemId: string, listId: string): Promise<ItemServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: !item ? "This item does not exist" : `Item retrieved from list ${listId}`,
      payload: { rows: item || Object.assign([]), rowCount: 1 },
    };
  }
  ...
  static async updateItem(id: string, description: string, checked: boolean): Promise<ServiceResponseInterface> {
    ...
    return result
      ? { status: "success", message: "item has been updated in List" }
