...
class ListServiceMongo {
  static async createList(name: string): Promise<ServiceResponseInterface> {
    ...
    return result
      ? { status: "success", message: "New List has been added successfully" }
      : { status: "failure", message: "This list cannot be created", code: 500 };
  }
  ...
  static async getLists(): Promise<ListServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: lists?.length === 0 ? "No list found" : "All lists",
      payload: { rows: lists || Object.assign([]), rowCount: lists?.length },
    };
  }
  ...
  static async getList(id: string): Promise<ListServiceResponseInterface> {
    ...
    return {
      status: "success",
      message: !list ? "This list does not exist" : `List ${id} returned`,
      payload: { rows: list || Object.assign([]), rowCount: 1 },
    };
  }
  ...
  static async deleteList(id: string): Promise<ServiceResponseInterface> {
    ...
    if (result && result.deletedCount) {
      ...
      code = 500;
      status = "failure";
      message = `Failed to delete List with id ${id}`;
  ...
  static async updateList(id: string, name: string): Promise<ServiceResponseInterface> {
    ...
    return result
      ...
      : { status: "failure", message: "update not done", code: 500 };
  }
}
