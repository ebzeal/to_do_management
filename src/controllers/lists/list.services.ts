...
import { addList, selectAListByParam, deleteAList, updateAList, selectAllLists } from '../../models/postgres/sqlQueries';
...
class ListService{
  static async createList(name: string): Promise<ServiceResponseInterface> {
    const { rows } = await query(selectAListByParam('name'), [name]);
      if (rows[0]) return {status:'failure', message:'This list already exists', code:409};
  ...
  static async deleteList(id: string): Promise<ServiceResponseInterface> {
    ...
    return {status: 'failure', message: 'list does not exist', code:404}
    }
